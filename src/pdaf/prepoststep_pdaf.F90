...
!! * 2026    - Anna Broschke- Adapted for flexible state vector 
!!
...
! ***************************************************
  ...
  if (id%ssh >0 .and. id%salt >0 .and. id%temp >0) call correct_state(step, dim_ens, ens_p)
...
! save values at forecast phase
  ...
  store_fcst: if ((step-step_null)<0) then
     ...
     ! -- Chlorophyll --
     ! save mean_chl_cci_p
     if (id%PhyChl >0 .and. id%DiaChl) then
...
! *****************************************************************
  ...
  if (id%ssh > 0) CALL store_stddev(step, stdev_p)
  ...
  ! Display RMS errors
  if (mype_filter==0) then
          if ( id%CO2f >0 .and. id%pCO2s >0 .and. id%temp >0 .and. id%DIC >0 .and. id%Alk >0 ) then
                write (*,'(a, 10x,a)') &
